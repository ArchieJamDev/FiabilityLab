  ...
  private = list(
    ...
    .run = function() {
      ...
      ic_body <- paste0(
        ...
        p(tr(
          "&alpha;'s standard-error formula also assumes approximate multivariate normality and unidimensionality (a single common factor). FiabilityLab's assumptions check reuses the per-item Shapiro-Wilk battery and the parallel-analysis factor count for these — so the same three checks (tau-equivalence, normality, unidimensionality) that determine whether &alpha; is trustworthy are reported together in one panel, the way interRater reports ICC's normality/homoscedasticity/linearity checks together.",
          "La fórmula del error estándar del &alpha; también asume normalidad multivariada aproximada y unidimensionalidad (un solo factor común). La verificación de supuestos de FiabilityLab reutiliza la batería de Shapiro-Wilk por ítem y el conteo de factores del análisis paralelo para esto — así los mismos tres supuestos (tau-equivalencia, normalidad, unidimensionalidad) que determinan si el &alpha; es confiable se reportan juntos en un panel, de la misma forma en que interRater reporta juntos los supuestos de normalidad/homocedasticidad/linealidad del ICC."
        )),
